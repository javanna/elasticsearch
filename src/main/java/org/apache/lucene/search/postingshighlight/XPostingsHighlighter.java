 ...
 * passages as if they were documents in this corpus. It uses a {@link java.text.BreakIterator} to find
 * passages in the text; by default it breaks using {@link java.text.BreakIterator#getSentenceInstance(java.util.Locale)
...
public class XPostingsHighlighter {
  ...
  // TODO Lucene 4.9: remove this class once we upgrade; see
  // LUCENE-5717

  static {
    assert Version.CURRENT.luceneVersion == org.apache.lucene.util.Version.LUCENE_48: "Remove this class once we upgrade to Lucene 4.9 where LUCENE-5717 is fixed";
  }
  ...
  private Map<Integer,Object> highlightField(String field, String contents[], BreakIterator bi, BytesRef terms[], int[] docids, List<AtomicReaderContext> leaves, int maxPassages, Query query) throws IOException {
    ...
    if (analyzer != null) {
      automata = XMultiTermHighlighting.extractAutomata(query, field);
    }
    ...
    for (int i = 0; i < docids.length; i++) {
      ...
      if (automata.length > 0) {
        DocsAndPositionsEnum dp = XMultiTermHighlighting.getDocsEnum(analyzer.tokenStream(field, content), automata);
