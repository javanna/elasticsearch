...
public class TransportDeleteAction extends TransportShardReplicationOperationAction<DeleteRequest, DeleteRequest, DeleteResponse> {
        ...
        if (state.metaData().hasIndex(request.concreteIndex())) {
            ...
            if (mappingMd != null && mappingMd.routing().required()) {
                if (request.routing() == null) {
                    ...
                    indexDeleteAction.execute(new IndexDeleteRequest(request, request.concreteIndex()), new ActionListener<IndexDeleteResponse>() {
                        @Override
                        public void onResponse(IndexDeleteResponse indexDeleteResponse) {
                            ...
                            listener.onResponse(new DeleteResponse(request.concreteIndex(), request.type(), request.id(), version, found));
                        }
    ...
    @Override
    protected String concreteIndex(DeleteRequest request) {
        return request.concreteIndex();
    }

    @Override
    protected String replicaConcreteIndex(DeleteRequest request) {
        return request.concreteIndex();
    }
    ...
    protected PrimaryResponse<DeleteResponse, DeleteRequest> shardOperationOnPrimary(ClusterState clusterState, PrimaryOperationRequest shardRequest) {
        ...
        DeleteResponse response = new DeleteResponse(request.concreteIndex(), request.type(), request.id(), delete.version(), delete.found());
    ...
    protected ShardIterator shards(ClusterState clusterState, DeleteRequest request) {
        return clusterService.operationRouting()
                .deleteShards(clusterService.state(), request.concreteIndex(), request.type(), request.id(), request.routing());
    }
}
