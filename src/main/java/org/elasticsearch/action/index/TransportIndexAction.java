...
public class TransportIndexAction extends TransportShardReplicationOperationAction<IndexRequest, IndexRequest, IndexResponse> {
    ...
    protected boolean resolveRequest(ClusterState state, IndexRequest request, ActionListener<IndexResponse> indexResponseActionListener) {
        ...
        request.process(metaData, mappingMd, allowIdGeneration);
        return true;
    }
    ...
    @Override
    protected String concreteIndex(IndexRequest request) {
        return request.concreteIndex();
    }

    @Override
    protected String replicaConcreteIndex(IndexRequest request) {
        return request.concreteIndex();
    }
    ...
    protected ShardIterator shards(ClusterState clusterState, IndexRequest request) {
        return clusterService.operationRouting()
                .indexShards(clusterService.state(), request.concreteIndex(), request.type(), request.id(), request.routing());
    }
    ...
    protected PrimaryResponse<IndexResponse, IndexRequest> shardOperationOnPrimary(ClusterState clusterState, PrimaryOperationRequest shardRequest) {
        ...
        IndexService indexService = indicesService.indexServiceSafe(shardRequest.request.concreteIndex());
        ...
        if (request.opType() == IndexRequest.OpType.INDEX) {
            ...
            if (index.parsedDoc().mappingsModified()) {
                mappingUpdatedAction.updateMappingOnMaster(request.concreteIndex(), index.docMapper(), indexService.indexUUID());
            }
            ...
            if (create.parsedDoc().mappingsModified()) {
                mappingUpdatedAction.updateMappingOnMaster(request.concreteIndex(), create.docMapper(), indexService.indexUUID());
            }
        ...
        IndexResponse response = new IndexResponse(request.concreteIndex(), request.type(), request.id(), version, created);
        return new PrimaryResponse<>(shardRequest.request, response, op);
    }

    @Override
    protected void shardOperationOnReplica(ReplicaOperationRequest shardRequest) {
        IndexShard indexShard = indicesService.indexServiceSafe(shardRequest.request.concreteIndex()).shardSafe(shardRequest.shardId);
