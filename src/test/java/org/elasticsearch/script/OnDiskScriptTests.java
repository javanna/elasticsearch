...
@RequiresScripts(context = ScriptContext.SEARCH, lang = {GroovyScriptEngineService.NAME, ExpressionScriptEngineService.NAME})
public class OnDiskScriptTests extends ElasticsearchIntegrationTest {
    ...
    public void testPartiallyDisabledOnDiskScripts() throws ExecutionException, InterruptedException {
        ...
        String query = "{ \"query\" : { \"match_all\": {}} , \"script_fields\" : { \"test1\" : { \"file\" : \"script1\", \"lang\":\"expression\" }}, size:1}";
        SearchResponse searchResponse = client().prepareSearch().setSource(query).setIndices("test").setTypes("scriptTest").get();
        assertHitCount(searchResponse, 5);
        assertTrue(searchResponse.getHits().hits().length == 1);
        SearchHit sh = searchResponse.getHits().getAt(0);
    ...
    @Test
    public void testDisabledOnDiskAggsScriptsBackwardCompat() {
        //test that script loaded from disk, although presented as inline (bw comp) is validated as a file one
        client().prepareIndex("test", "scriptTest", "1").setSource("{\"theField\":\"foo\"}").get();
        refresh();
        String source = "{\"aggs\": {\"test\": { \"terms\" : { \"script\":\"script1\", \"lang\": \"expression\" } } } }";
        try {
            client().prepareSearch("test").setSource(source).get();
            fail("aggs script should have been rejected");
        } catch(Exception e) {
