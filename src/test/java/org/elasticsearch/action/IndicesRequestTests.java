...
public class IndicesRequestTests extends ElasticsearchIntegrationTest {
    ...
    @Test
    public void testIndicesStatus() {
        String indicesStatus = IndicesStatusAction.NAME + "[s]";
        interceptTransportActions(indicesStatus);

        IndicesStatusRequest indicesStatusRequest = new IndicesStatusRequest(randomIndicesOrAliases());
        internalCluster().clientNodeClient().admin().indices().status(indicesStatusRequest).actionGet();

        clearInterceptedActions();
        assertSameIndices(indicesStatusRequest, indicesStatus);
    }
