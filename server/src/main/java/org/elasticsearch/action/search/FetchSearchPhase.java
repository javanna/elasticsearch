...
final class FetchSearchPhase extends SearchPhase {
    ...
    private void innerRun() throws IOException {
        ...
        if (queryAndFetchOptimization) {
            ...
            final IntArrayList[] docIdsToLoad = searchPhaseController.fillDocIdsToLoad(numShards, reducedQueryPhase.topDocs.scoreDocs);
            if (reducedQueryPhase.topDocs.scoreDocs.length == 0) { // no docs to fetch -- sidestep everything and return
                phaseResults.stream()
