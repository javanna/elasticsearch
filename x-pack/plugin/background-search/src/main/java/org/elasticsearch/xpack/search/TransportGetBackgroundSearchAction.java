    ...
    protected void doExecute(Task task, GetTaskRequest request, ActionListener<GetBackgroundSearchResponse> listener) {
        ...
        client.admin().cluster().getTask(request, new ActionListener<GetTaskResponse>() {
            ...
            public void onResponse(GetTaskResponse getTaskResponse) {
                ...
                if (status instanceof TransportSubmitBackgroundSearchAction.BackgroundSearchStatus) {
                    TransportSubmitBackgroundSearchAction.BackgroundSearchStatus bgStatus =
                        (TransportSubmitBackgroundSearchAction.BackgroundSearchStatus) status;
                    listener.onResponse(new GetBackgroundSearchResponse(bgStatus.getTotalShards(),
                        bgStatus.getCompletedOperations(), bgStatus.getPartialAggs()));
                } else {
                    assert taskResult.isCompleted();
                    listener.onResponse(new GetBackgroundSearchResponse(taskResult.getResponse()));
                }
            }
